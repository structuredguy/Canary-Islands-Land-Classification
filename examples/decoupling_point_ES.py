...
plt.plot(years, tourist_demand, 'k--', linewidth=2, label='Demanda Turística Real (Noches de Estancia)')

# Plot Healthy Supply
plt.plot(years, hotel_supply, 'g-', linewidth=2, label='Adaptación (Capacidad  Hotelera)')

# Plot Speculative Supply
plt.plot(years, speculative_supply, 'r-', linewidth=2, label='Construcción (Boom + Especulación)')

# Fill the "Speculation Gap"
plt.fill_between(years, hotel_supply, speculative_supply, where=(speculative_supply > hotel_supply),
                 color='red', alpha=0.2, hatch='//', label='La "Cuña de la Especulación"')

# Annotate the 1973 Oil Crisis
plt.axvline(x=1973, color='gray', linestyle=':')
plt.text(1973.5, 100, '1973 Crisis del Petròleo\n(Cae la demanda, continúa la Construcción)', fontsize=10)

plt.title('Loa Años 1970s: Distiguendo Adaptación de Especulación')
plt.ylabel('Índice (1965=10)')
plt.legend()
plt.grid(True, alpha=0.3)
# %%
plt.savefig('../img/decoupling_point_ES.png')
